...
export default function GlobalAttendanceListener() {
  ...
  // Data Caches
  const [coursesCache, setCoursesCache] = React.useState<Map<string, any>>(
    new Map(),
  );
      ...
      try {
        ...
        if (message.type === "ESP32_STATUS") {
          ...
          const timeSinceLastProcess =
            now - lastProcessedVerification.current.timestamp;
          ...
          if (!isDuplicate) {
            ...
            const lat =
              message.latitude || message.lat || lastKnownGPS.current.latitude;
            const lon =
              message.longitude ||
              message.lon ||
              lastKnownGPS.current.longitude;
            ...
            console.log(
              `⚠️ Skipping duplicate verification for ID ${verificationId}`,
            );
          }
        }
      } catch (e) {
  ...
  const handleAttendance = async (data: any) => {
    ...
    if (globalPendingAttendance.has(lockKey)) {
      console.warn(
        `🔒 Blocked duplicate processing for ${studentData.name} (Global Locked)`,
      );
    ...
    try {
      ...
      (async () => {
        ...
        console.log(
          `✅ Attendance Logged: ${studentData.name} | 📍 ${locationName}`,
        );
      })()
        .then(() => globalPendingAttendance.delete(lockKey))
        .catch((err) => {
