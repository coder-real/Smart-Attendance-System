...
export default function FingerprintPortal() {
  ...
  const [gpsStatus, setGpsStatus] = useState<"searching" | "ready" | "offline">(
    "offline",
  );
    ...
    ws.current.onmessage = (event) => {
      ...
      try {
        ...
        if (jsonData.type === "ATTENDANCE") {
          ...
          setCardData({
            name:
              jsonData.studentName || jsonData.data?.studentName || "Student",
            studentId: jsonData.studentId || jsonData.data?.studentId || "N/A",
            department:
              jsonData.department || jsonData.data?.department || "N/A",
            courseName:
              jsonData.courseName || jsonData.data?.courseName || "Course",
            attendancePercentage:
              jsonData.attendancePercentage ||
              jsonData.data?.attendancePercentage ||
              0,
            status: "entry",
          });
          return;
  ...
  const handleStartVerification = () => {
    ...
    if (gpsStatus !== "ready") {
      setStatusModal({
        ...
        title: "GPS Offline",
        message:
          "Waiting for satellite synchronization. Please wait for GPS Lock.",
      });
      return;
    }
